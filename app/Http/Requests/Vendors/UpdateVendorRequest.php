...
class UpdateVendorRequest extends FormRequest
{
    ...
    {
        $kind = $this->input('kind', $this->route('vendor')?->kind?->value ?? VendorKind::Active->value);

        return [
            'name' => ['sometimes', 'nullable', 'string', 'max:255'],
            'kind' => ['sometimes', Rule::enum(VendorKind::class)],
            'status' => ['sometimes', 'string', Rule::in($this->allowedStatusesForKind($kind))],
